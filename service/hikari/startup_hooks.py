"""Lifecycle hooks for the Hikari bot at startup and shutdown."""
import logging
import time
import typing
...
async def initialize_wordle_messages(bot: hikari.GatewayBot) -> None:
    ...
    try:
        ...
        def is_wordle_message(message: Message) -> bool:
            return message.author.id == Snowflake(wordle_user_id)

        messages = await _search_user_messages(bot, channel_id, is_wordle_message)
